...
import { FaFacebook, FaInstagram, FaTwitter, FaTiktok, FaPlay  } from "react-icons/fa";
...
export default function MainContentSection() {
  ...
  useEffect(() => {
    ...
    // Facebook iframe dan Snack Video tidak perlu script loading
    if (currentPlatform.name === "Facebook" || currentPlatform.name === "Snack Video") {
      return;
    }
    ...
    if (!existingScript && currentPlatform.scriptUrl) {
      const script = document.createElement("script");
      script.id = scriptId;
      script.src = currentPlatform.scriptUrl;
      script.async = true;

      if (currentPlatform.name === "Twitter/X") {
        script.setAttribute("charset", "utf-8");
      }

      document.body.appendChild(script);
      
      // Cleanup setelah script loaded
      script.onload = () => {
        if (currentPlatform.name === "Instagram" && window.instgrm) {
          setTimeout(() => window.instgrm?.Embeds.process(), 100);
        } else if (currentPlatform.name === "Twitter/X" && window.twttr) {
          setTimeout(() => window.twttr?.widgets.load(), 100);
        }
      };
    } else if (existingScript) {
      // Jika script sudah ada, trigger render ulang
      if (currentPlatform.name === "Instagram" && window.instgrm) {
        setTimeout(() => window.instgrm?.Embeds.process(), 100);
      } else if (currentPlatform.name === "Twitter/X" && window.twttr) {
        setTimeout(() => window.twttr?.widgets.load(), 100);
            ...
            <div>
              ...
              <Card className="border border-gray-200 bg-white rounded-xl md:rounded-2xl shadow-md transition-all duration-300">
                <CardContent className="p-3 md:p-4">
                  {/* Social Media Tabs dengan Icon - Persegi */}
                  <div className="flex items-center justify-center gap-2 md:gap-2.5 mb-3 md:mb-4 overflow-x-auto pb-1 p-4">
                        ...
                        <button
                          ...
                          className={`p-2 md:p-2.5 rounded-lg transition-all duration-300 flex-shrink-0 ${
                            activePlatform === index
                              ? "shadow-md scale-105"
                              : "hover:scale-105 bg-gray-100 hover:bg-gray-200"
                          }`}
                          style={{
                            backgroundColor: activePlatform === index ? platform.color : undefined,
                          }}
                          title={platform.name}
                        >
                          <IconComponent
                            className={`w-6 h-6 md:w-7 md:h-7 ${
                              activePlatform === index ? "text-white" : "text-gray-600"
                            }`}
                          />
                        </button>
                      );
